...
export function calcTargetsForGoal(profile: TargetProfile, goal: Goal): FoodTargets {
  ...
  // Protein per kg of lean body mass, not total bodyweight — total-BW targets
  // overshoot for high-BF users. ~2.8 g/kg LBM on a cut, ~2.5 otherwise
  // (roughly equivalent to 2.5/2.2 g/kg total BW for a lean lifter).
  const lbm      = profile.weightKg * (1 - profile.currentBf / 100);
  const proteinG = Math.round(lbm * (goal === "cut" ? 2.8 : 2.5));
